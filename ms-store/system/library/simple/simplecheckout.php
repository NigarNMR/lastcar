...
class SimpleCheckout extends Simple {
    ...
    public function __construct($registry, $settingsId = 0) {
    ...
    public function preparePaymentMethod($method) {
        ...
        if (!$this->isPaymentBeforeShipping() && $this->cart->hasShipping(1) && empty($checkedMethod['forAllMethods']) && !empty($checkedMethod['forMethods'])) {
            $shippingMethod = isset($this->session->data['shipping_method']) ? $this->session->data['shipping_method'] : array();;
    ...
    public function displayAddressSame() {
        return $this->cart->hasShipping(1) && !$this->isBlockHidden('shipping_address') && !$this->isBlockHidden('payment_address') && $this->getSettingValue('displayAddressSame', 'payment_address');
    }
    ...
    public function loadFromCookies() {
        if (isset($this->request->cookie['simple'])) {
    ...
    private function saveToCookies() {
        ...
        setcookie('simple', base64_encode(serialize($simple)), time() + 60 * 60 * 24 * 30);
    }
    ...
    public function isShippingAddressEmpty() {
        ...
        foreach ($fields as $fieldSettings) {
            if ($this->isFieldUsed($fieldSettings['id'], 'shipping_address') && in_array($fieldSettings['id'], $this->_observedFields['shipping_address']) && empty($this->session->data['simple']['shipping_address'][$fieldSettings['id']])) {
                $empty = true;
            }
        }
    ...
    public function isPaymentAddressEmpty() {
        ...
        foreach ($fields as $fieldSettings) {
            if ($this->isFieldUsed($fieldSettings['id'], 'payment_address') && in_array($fieldSettings['id'], $this->_observedFields['payment_address']) && empty($this->session->data['simple']['payment_address'][$fieldSettings['id']])) {
                $empty = true;
            }
        }
    ...
    private function copyNameFields() {
        foreach (array('firstname', 'lastname') as $field) {
            if (!$this->isFieldUsed($field, 'customer')) {
