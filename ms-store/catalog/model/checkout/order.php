...
class ModelCheckoutOrder extends Model {
        ...
        if (isset($data['products'])) {
            foreach ($data['products'] as $product) {
                ...
                foreach ($product['option'] as $option) {
                    if ($option['name'] === 'Price_Original') {
                        $acquiring_price = $option['value'];
                    }
                }
                
                $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int) $order_id . "', product_id = '" . (int) $product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int) $product['quantity'] . "', price = '" . (float) $product['price'] . "', total = '" . (float) $product['total'] . "', tax = '" . (float) $product['tax'] . "', reward = '" . (int) $product['reward'] . "', acquiring_price = '" . (float) $acquiring_price . "'");
                ...
                // @note сохраняем стандартный статус для товара в заказе
                $order_product_status_id = $this->config->get('config_order_product_status_id');

                $this->addOrderProductHistory($order_product_id, $order_product_status_id);
            }
        }
    ...
    /*
     * @note получить подробную информацию о товаре в заказе
     * нужно для изменения статуса
     */
    ...
    public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        if ($order_info) {
            ...
            if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                $data['text_day'] = $language->get('text_new_day');
                $data['text_article'] = $language->get('text_new_article');
    ...
    public function addOrderProductHistory($order_product_id, $order_product_status_id, $comment = '', $notify = false) {
        $order_product_info = $this->getOrderProduct($order_product_id);

        // @note для чего снова проверка?
        if ($order_product_info) {

            // @note begin Мой код mashkovsky_ab@eniak.ru
            // Update the DB with the new statuses
            $this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET order_product_status_id = '" . (int) $order_product_status_id . "' WHERE order_product_id = '" . (int) $order_product_id . "'");

            $this->db->query("INSERT INTO " . DB_PREFIX . "order_product_history SET order_product_id = '" . (int) $order_product_id . "', order_product_status_id = '" . (int) $order_product_status_id . "', notify = '" . (int) $notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
            // @note end Мой код
            // выход из функции, т.к. не нужно больше ничего делать
            return true;
