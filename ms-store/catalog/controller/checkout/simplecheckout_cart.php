...
class ControllerCheckoutSimpleCheckoutCart extends SimpleController {
    ...
    public function index() {
        ...
        if (!empty($minAmount) || !empty($maxAmount)) {
            if ($useTotal) {
                $cartSubtotal = $this->cart->getTotal(1);
            } else {
                $cartSubtotal = $this->cart->getSubTotal(1);
            }
        }
        ...
        $cartQuantity = $this->cart->countProducts(1);
        ...
        $this->_templateData['column_article'] = $this->language->get('column_article');
        $this->_templateData['column_comment'] = $this->language->get('column_comment');
      ...
      print '</pre>';   */
                if ($version < 220) {
                        ...
                        $this->{'model_total_' . $result['code']}->getTotal($total_data, 1);
                    }
...
print '<pre>';
      ...
      print '</pre>';*/
        ...
        if ($currentTheme == 'shoppica' || $currentTheme == 'shoppica2') {
            ...
            $this->_templateData['cart_total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->simplecheckout->formatCurrency($total));
        }
        ...
        $this->_templateData['display_error'] = $this->simplecheckout->displayError('cart');
    ...
    private function validateReward() {
        ...
        if (!empty($this->request->post['reward'])) {
            ...
            foreach ($this->cart->getProducts(1) as $product) {
                if ($product['points']) {
                    $points_total += $product['points'];
                }
            }
