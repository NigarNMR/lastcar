...
class ControllerCheckoutSimpleCheckout extends SimpleController {
    ...
    public function index($args = null) {
        ...
        $this->_templateData['cart_empty']              = !$this->cart->hasProducts(1) && empty($this->session->data['vouchers']);
    ...
    private function order() {
        ...
        if ($version < 152) {
            ...
            $data['total'] = $total;
            $data['reward'] = $this->cart->getTotalRewardPoints(1);
        } elseif ($version >= 152) {
            foreach ($this->cart->getProducts(1) as $product) {
 ...
 //////////////////////////// проверка на tecdoc
        ...
        // Хардкод; исправить по возможности
