...
class ControllerMarketingMarketing extends Controller {
	...
	protected function getList() {
		...
		$pagination->limit =1;// $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('marketing/marketing', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($marketing_total) ? (($page - 1) * 2) + 1 : 0, ((($page - 1) * 2) > ($marketing_total - 2)) ? $marketing_total : ((($page - 1) * 2) + 2), $marketing_total, ceil($marketing_total / 2));
