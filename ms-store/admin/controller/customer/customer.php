...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
        ...
        $data['refresh'] = $this->url->link('customer/customer/refresh', 'token=' . $this->session->data['token'], true);
       ...
       //  var_dump($results);
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'approve'        => $approve,
				'unlock'         => $unlock,
				'edit'           => $this->url->link('customer/customer/edit', 'token=' . $this->session->data['token'] . '&customesdsssdr_id=' . $result['customer_id'] . $url, true),
                'balance_now'        => $this->currency->format($this->model_customer_customer->getBalanceNow($result['customer_id']),  $this->config->get('config_currency')),
                'balance_freeze'        => $this->currency->format($this->model_customer_customer->getBalanceFreeze($result['customer_id']),  $this->config->get('config_currency')),
                'balance_limit'        => $this->currency->format($this->model_customer_customer->getBalanceLimit($result['customer_id']),  $this->config->get('config_currency')),
            );
		}                                             

//
//		$results = $this->model_customer_customer->getTransactions($result['customer_id']);
//
//		foreach ($results as $result) {
//			$data['customers'][] = array(
//                            'customer_id'    => $result['customer_id'],
...
//		}
//                                echo '<pre>';
//                var_dump($data['transactions'][1]['amount'] +$data['transactions'][0]['amount']);
//                echo '</pre>';
//                                
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['limit'])) {
            ...
            $data['limit'] = $customer_info['balance_limit'];
        } else {
			$data['limit'] = 0;
		}
	...
	protected function validateForm() {
        ...
        if (!is_numeric($this->request->post['limit'])) {
			$this->error['limit'] = $this->language->get('error_limit_format');
		} elseif (is_numeric($this->request->post['limit']) && (intval($this->request->post['limit']) < 0)) {
            $this->error['limit'] = $this->language->get('error_limit_negative');
        }
	...
	public function transaction() {
        ...
        $data['text_balance_total'] = $this->language->get('text_balance_total');
        ...
        if (isset($this->request->get['tab'])) {
			$tab_active = $this->request->get['tab'];
		} else {
			$tab_active = 'current';
		}
        ...
        {
            case 'current':
                $tab_operations_code = 1;
                break;
            case 'reserve':
                $tab_operations_code = 2;
                break;
            default:
                break;
        }
        ...
        $data['page_limit_url'][1] = $this->url->link(
            ...
            'token=' . $this->session->data['token'] 
                ...
                . '&date-end=' . $date_end,
            true);
        $data['page_limit_url'][2] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
                ...
                . '&date-end=' . $date_end,
            true);
        $data['page_limit_url'][3] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
                ...
                . '&date-end=' . $date_end,
            true);
        $data['page_limit_url'][4] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
                ...
                . '&date-end=' . $date_end,
            true);
        $data['page_limit_url'][5] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
        ...
        $data['page_tab_text'][1] = $this->language->get('text_tab_current');
        $data['page_tab_text'][2] = $this->language->get('text_tab_reserve');
        $data['page_tab_value'] = $this->language->get('text_tab_' . $tab_active);
        $data['page_tab_code'] = $tab_active;
        
        $data['page_tab_url'][1] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
                ...
                . '&date-end=' . $date_end,
            true);
        $data['page_tab_url'][2] = $this->url->link(
            'customer/customer/transaction',
            'token=' . $this->session->data['token'] 
                . '&customer_id=' . $this->request->get['customer_id']
                . '&page=1' . ''
        ...
        if (!empty($date_start) && !empty($date_end)) {
            $data['datepicker_value'] = $date_start . ' - ' . $date_end;
        } else {
            $data['datepicker_value'] = '';
        }
        
		$data['transactions'] = array();

        $filter = array(
            'ops_code' => $tab_operations_code,
            'date_start' => $date_start,
            'date_end' => $date_end
        );
        
		$results = $this->model_customer_customer->getTransactions($this->request->get['customer_id'], ($page - 1) * $page_limit, $page_limit, $filter);

		foreach ($results as $result) {
            if (intval($result['amount']) > 0) {
                $operation_info = $this->language->get('text_operation_increase_' . intval($result['operation_status']));
            } else {
                $operation_info = $this->language->get('text_operation_decrease_' . intval($result['operation_status']));
            }
            //Указание данных по товару, если есть
            if (empty($result['order_product_id']) || is_null($result['order_product_id'])) {
                $product_description = 'N/A';
            } else {
                $product_description = $this->model_customer_customer->getOrderProductName($result['order_product_id']);
            }
			$data['transactions'][] = array(
				'amount'      => $this->currency->format($result['amount'], $this->config->get('config_currency')),
		...
		$data['balance'] = $this->currency->format($this->model_customer_customer->getTransactionTotal($this->request->get['customer_id'], $filter), $this->config->get('config_currency'));
        
        $data['token'] = $this->request->get['token'];
        $data['customer_id'] = $this->request->get['customer_id'];

		$transaction_total = $this->model_customer_customer->getTotalTransactions($this->request->get['customer_id'], $filter);
    ...
    public function refresh() {
		...
		if (isset($this->request->get['selected'])) {
			foreach ($this->request->get['selected'] as $customer_id) {
				$this->model_customer_customer->refreshCustomerBalance($customer_id);
			}
    ...
    public function export() {
        ...
        $name_hash = md5($this->request->get['customer_id'] . time() . $date_start . $date_end);
        ...
        $results = $this->model_customer_customer->getTransactions($this->request->get['customer_id'], 0, 0, $filter_data, false);
