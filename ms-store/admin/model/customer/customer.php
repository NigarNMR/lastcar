...
class ModelCustomerCustomer extends Model {
    ...
    public function editCustomer($customer_id, $data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int) $data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', newsletter = '" . (int) $data['newsletter'] . "', status = '" . (int) $data['status'] . "', approved = '" . (int) $data['approved'] . "', safe = '" . (int) $data['safe'] . "', balance_limit = '" . $data['limit'] . "' WHERE customer_id = '" . (int) $customer_id . "'");
    ...
    public function addTransaction($customer_id, $description = '', $amount = '', $order_id = 0) {
        ...
        if ($customer_info) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "customer_transaction SET customer_id = '" . (int) $customer_id . "', order_product_id = '" . (int) $order_id . "', description = '" . $this->db->escape($description) . "', amount = '" . (float) $amount . "', date_added = NOW(), operation_status = '1', payment_code = '1'");

            $this->db->query("UPDATE " . DB_PREFIX . "customer SET balance_now = balance_now + '" . $amount . "' WHERE customer_id = '" . $customer_id . "'");
    ...
    public function getTransactions($customer_id, $start = 0, $limit = 10, $data = array(), $pagination_switch = true) {
        if ($start < 0) {
            $start = 0;
        }
        ...
        if (isset($data['ops_code']) && !empty($data['ops_code'])) {
            $sql .= " AND operation_status = '" . $data['ops_code'] . "'";
        }
        ...
        if ($pagination_switch) {
            $sql .= " DESC LIMIT " . (int) $start . "," . (int) $limit;
        }
    ...
    public function getTotalTransactions($customer_id, $data = array()) {
        $sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . (int) $customer_id . "'";
    ...
    public function getTransactionTotal($customer_id, $data = array()) {
        $sql = "SELECT SUM(amount) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . (int) $customer_id . "'";
    ...
    public function getBalanceNow($customer_id) {
        $query = $this->db->query("SELECT balance_now AS balance FROM " . DB_PREFIX . "customer WHERE customer_id='" . $customer_id . "'");
        return $query->row['balance'];
    }
    
    public function getBalanceFreeze($customer_id) {
        $query = $this->db->query("SELECT balance_freeze AS balance FROM " . DB_PREFIX . "customer WHERE customer_id='" . $customer_id . "'");
        return $query->row['balance'];
    }
    
    public function getBalanceLimit($customer_id) {
        $query = $this->db->query("SELECT balance_limit AS balance FROM " . DB_PREFIX . "customer WHERE customer_id='" . $customer_id . "'");
        return $query->row['balance'];
    }
    ...
    public function refreshCustomerBalance($customer_id) {
        $sql = "SELECT SUM(amount) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . $customer_id . "' AND operation_status='1'";
        $query = $this->db->query($sql);
        $balance_now = $query->row['total'];
        
        $sql = "SELECT SUM(amount) AS total FROM " . DB_PREFIX . "customer_transaction WHERE customer_id = '" . $customer_id . "' AND operation_status='2'";
        $query = $this->db->query($sql);
        $balance_freeze = $query->row['total'];
        
        $sql = "UPDATE " . DB_PREFIX . "customer SET balance_now = '" . $balance_now . "', balance_freeze = '" . $balance_freeze . "' WHERE customer_id = '" . $customer_id . "'";
        $query = $this->db->query($sql);
    }
}
